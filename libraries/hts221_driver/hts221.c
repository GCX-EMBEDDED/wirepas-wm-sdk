...
{
    uint8_t reg_val;
    htc221_readI2C(WHO_AM_I_REG, &reg_val, 1);
    LOG(LVL_DEBUG, "I2C read. WHO_AM_I_VAL: 0x%x", reg_val);
...
int16_t hts221_humidity_get(void)
{
    ...
    htc221_readI2C(HUMIDITY_OUT_L_REG, &humid_l, 1);
    htc221_readI2C(HUMIDITY_OUT_H_REG, &humid_h, 1);
...
float hts221_temperature_get(void)
{
    ...
    htc221_readI2C(TEMP_OUT_L_REG, &temp_l, 1);
    htc221_readI2C(TEMP_OUT_H_REG, &temp_h, 1);
...
int32_t hts221_one_shot(void)
{
    ...
    err = htc221_readI2C(CTRL_REG2, &reg_val, 1);
...
int32_t hts221_cfg_set(hts221_cfg_t *p_cfg)
{
    int32_t err;
    ...
    err = htc221_writeI2C(AV_CONF_REG, &(p_cfg->av_conf), 1);
    err = htc221_writeI2C(CTRL_REG1, &(p_cfg->ctrl_reg1), 1);
    err = htc221_writeI2C(CTRL_REG2, &(p_cfg->ctrl_reg2), 1);
...
int32_t htc221_init(void)
{
    ...
    err = hts221_calib_get(&htc221_sensor.calib);
